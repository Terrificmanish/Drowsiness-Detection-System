...
function stopDetection() {
  ...
  isCalibrationRunning = false; // Ensure calibration is stopped if detection stops
  videoElement.pause();
...
function startCalibration() {
  ...
  function collectCalibrationData() {
    ...
    if (Date.now() - startTime > CALIBRATION_DURATION) {
      ...
      setTimeout(collectCalibrationData, 1000); // Collect data every second
    }
  }
...
function stopCalibration() {
  ...
  // Process calibrationData to adjust EAR threshold or other parameters
}
